...
def fc_forward(x, w, b):
    ...
    # Flatten each input example into a row
    x_flat = x.reshape(N, -1) #shape(N, D)

    # Affine transformation
    out = x_flat.dot(w) + b #shape(N, M)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = (x, w, b)
...
def fc_backward(dout, cache):
    ...
    dx = dx_flat.reshape(x.shape) # reshape back to (N, d_1, ..., d_k)
    dw = x_flat.T.dot(dout)   # shape(D, M)
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        ###########################################################################
        # Probability of keeping a neuron is (1 - p)
